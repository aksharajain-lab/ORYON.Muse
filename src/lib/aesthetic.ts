 ...
 *
 * Replies are no longer templated here — the guide is a real LLM
 * conversation served by POST /api/chat (see lib/gemini.server.ts). These
 * exports remain: the suggestion prompts, and the MuseReply shape that the
 * chat endpoint returns and the guide UI renders (Observation → Insight →
 * Direction sections, with an occasional "Muse moment" highlight).
 * ─────────────────────────────────────────────────────────────────────── */
